  ...
  def self.up
    ...
    ClientStore.all.each do |client_store|
      if client_store.data
        ...
        client_store.data = client_store.data.gsub /\{"name":"request_transaction\[grant_sub_program_ids\]\[\]","value":"(\d*)"\}/, '{"name":"request[request_hierarchy][]","value":"-\1--"}'
        client_store.data = client_store.data.gsub /\{"name":"request_transaction\[grant_program_ids\]\[\]","value":"(\d*)"\}/, '{"name":"request[request_hierarchy][]","value":"\1---"}'
